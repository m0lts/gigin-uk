...
export async function markPendingFeeInDispute({ musicianId, docId, gigId, disputeReason, details, venueId }) {
    try {
      const fn = httpsCallable(functions, "markPendingFeeInDispute");
      const { data } = await fn({ musicianId, docId, gigId, disputeReason, details, venueId });
      return data;
    } catch (error) {
