...
import { MoreInformationIcon, PeopleGroupIconSolid, TechRiderIcon, EditIcon, AddMember, LeftChevronIcon, RightArrowIcon, PlusIconSolid, CloseIcon, ExitIcon, VocalsIcon, DrumsIcon, KeysIcon, GuitarsIcon, BassIcon, SaxIcon, TrumpetIcon, TromboneIcon, PlaybackIcon, MusicianIconSolid, MicrophoneLinesIcon, PlugIcon, SpeakersIcon, SlidersIcon, AmpIcon, LinkIcon } from "../../../shared/ui/extras/Icons";
...
// Instrument-specific question configurations (must match ProfileCreationBox)
...
export const AdditionalInfoSection = ({ type, onClose, profileData, profileId, canEdit = true, isOwner = false }) => {
  ...
  useEffect(() => {
    ...
    if (profileData?.techRider) {
      ...
      setTechRiderData({
        ...
        paSource: techRider.paSource ?? (techRider.canBringPA ? 'own' : 'venue'),
        mixingDeskSource: techRider.mixingDeskSource ?? (techRider.canBringMixingDesk ? 'own' : 'venue'),
        bringableGearNotes: techRider.bringableGearNotes ?? '',
      });
  ...
  const handleSetPerformerCount = () => {
    const currentLen = (techRiderData.lineup || []).length;
    const raw = performerCountInput !== '' && performerCountInput !== undefined ? performerCountInput : currentLen;
    ...
    setTechRiderData(prev => {
      const prevLen = (prev.lineup || []).length;
      if (prevLen > 0) {
        // Editing: preserve existing performers when changing count
        if (n > prevLen) {
          const added = Array.from({ length: n - prevLen }, newPerformer);
          return {
            ...prev,
            lineup: [...(prev.lineup || []), ...added],
            performerDetails: [...(prev.performerDetails || []), ...Array.from({ length: n - prevLen }, () => ({}))]
          };
        }
        if (n < prevLen) {
          const newLineup = (prev.lineup || []).slice(0, n);
          const newDetails = (prev.performerDetails || []).slice(0, n);
          const newPerformers = (prev.stageArrangement?.performers || []).filter(p => p.lineupIndex < n);
          return {
            ...prev,
            lineup: newLineup,
            performerDetails: newDetails,
            stageArrangement: { ...prev.stageArrangement, performers: newPerformers }
          };
        }
        return prev;
      }
      return {
        ...prev,
        lineup: Array.from({ length: n }, newPerformer),
        performerDetails: Array.from({ length: n }, () => ({}))
      };
    });
      ...
      return (
        <div className="additional-info-section">
          ...
          <div className="section-content">
            ...
            {(() => {
              const { performerCount, items } = getEquipmentSummary(techRiderData);
              ...
              return (
                <>
                  <h5 className="tech-rider-equipment-summary-title">Equipment Summary</h5>
                  <div className="tech-rider-equipment-summary">
                  ...
                  {items.length > 0 ? (
                    <ul className="tech-rider-equipment-summary-list">
                      {items.map((item, i) => (
                        <li key={i}>
                          <span className="tech-rider-equipment-summary-item-icon" aria-hidden>{getEquipmentIcon(item.type)}</span>
                          {item.text}
                        </li>
                      ))}
                    </ul>
            ...
            <div className="tech-rider-stage-map-container view-mode">
              <h5 className="tech-rider-equipment-summary-title">Stage Layout</h5>
            ...
            <h5 className="tech-rider-equipment-summary-title" style={{ marginTop: '1rem' }}>Requirements per performer</h5>
            <div className="tech-rider-viewer-performers-list">
              ...
              {techRiderData.extraNotes && techRiderData.extraNotes.trim() !== '' && (
                <>
                  <h5 className="tech-rider-equipment-summary-title" style={{ marginTop: '1rem' }}>Additional Notes</h5>
          ...
          {techRiderStage === 1 && (
            <div className="tech-rider-stage">
              {techRiderData.lineup.length === 0 ? (
                ...
                <>
                  {canEdit && (
                    <div className="tech-rider-performer-count-step" style={{ marginBottom: '1rem' }}>
                      <p className="creation-step-question">How many performers in the band?</p>
                      <div className="tech-rider-performer-count-input-wrap">
                        <input
                          type="number"
                          ...
                          min={1}
                          max={50}
                          value={performerCountInput !== '' && performerCountInput !== undefined ? performerCountInput : techRiderData.lineup.length}
                          onChange={(e) => setPerformerCountInput(e.target.value)}
                          onKeyDown={(e) => e.key === 'Enter' && handleSetPerformerCount()}
                          placeholder="e.g. 3"
                          aria-label="Number of performers"
          ...
          {techRiderStage === 2 && (
            <div className="tech-rider-stage">
              {techRiderData.lineup.length === 0 ? (
                ...
                <>
                  <p className="tech-rider-performer-progress-label">
                    {(() => {
                      ...
                      return (
                        <>
                          ...
                          <span className="tech-rider-performer-progress-count"> ({currentPerformerIndex + 1} of {techRiderData.lineup.length})</span>
                        </>
                      );
                    })()}
                  </p>
