...
export const ProfileForm = ({ user, musicianProfile, band = false, expand, setShowPreview, bandAdmin = true }) => {
    ...
    useEffect(() => {
        ...
        const checkForSavedProfile = async () => {
            ...
            if (isBand && band) {
                const { bandInfo, members, bandId, ...rest } = musicianProfile;
    ...
    const handleFileChange = async (event) => {
        ...
        try {
            ...
            sessionUploadsRef.current.videos.add(videoUrl);
            if (thumbnailUrl) sessionUploadsRef.current.thumbs.add(thumbnailUrl);
    ...
    const handleAudioUpload = async (event) => {
        ...
        try {
          ...
          sessionUploadsRef.current.tracks.add(audioUrl);
          const trackData = {
            title,
            file: audioUrl,
            date: today,
    ...
    const handlePhotoUpload = async (event) => {
        ...
        try {
          for (let i = 0; i < filesToUpload.length; i++) {
            ...
            try {
              const url = await uploadFileWithProgress(file, photoPath);
              setFormData(prev => {
                const next = [...(prev.photos || []), url].slice(0, MAX_PHOTOS);
                return { ...prev, photos: next };
              });
              sessionUploadsRef.current.photos.add(url);
    ...
    const handleDiscard = async () => {
          const deletions = [];
          for (const url of sessionUploadsRef.current.videos) {
            deletions.push(deleteFileFromStorage(url));
          }
          for (const url of sessionUploadsRef.current.thumbs) {
            deletions.push(deleteFileFromStorage(url));
          }
          for (const url of sessionUploadsRef.current.tracks) {
            deletions.push(deleteFileFromStorage(url));
          }
          for (const url of sessionUploadsRef.current.photos) {
            deletions.push(deleteFileFromStorage(url));
          }
          await Promise.allSettled(deletions);
          sessionUploadsRef.current.videos.clear();
          sessionUploadsRef.current.thumbs.clear();
          sessionUploadsRef.current.tracks.clear();
          sessionUploadsRef.current.photos.clear();
          setShowPreview(true);
        ...
        {    bandAdmin ? (
                <div className={`profile-form ${band ? 'band' : ''}`}>
                    ...
                    <div className="action-buttons">
                        ...
                        <button className="btn tertiary" onClick={handleDiscard}>
                            Discard Changes
                        </button>
                    </div>
                ...
                <div className="profile-form not-admin">
                    <PeopleGroupIconSolid />
                    <h4>Sorry, only the Band's Admin can edit the band's profile.</h4>
                    <button className="btn tertiary" onClick={() => setShowPreview(true)}>
                        View Band Profile
                    </button>
                </div>
            )}
