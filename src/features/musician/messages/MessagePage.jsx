...
export const MessagePage = () => {
    ...
    useEffect(() => {
        const list = showArchived ? archivedConversations : conversations;
              if (isMdUp && !paramsConversationId && list.length > 0) {
          navigate(`/messages?conversationId=${list[0].id}`);
        }
    }, [showArchived, archivedConversations, conversations, paramsConversationId, navigate, isMdUp]);
                            ...
                            <div className='column message-thread'>
                                <div className='top-banner'>
                                    ...
                                    <div className='buttons' style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap:5}}>
                                        <button className='btn secondary' onClick={(e) => openInNewTab(`/venues/${activeConversation.accountNames.find(account => account.role === 'venue')?.participantId}`, e)}>
                                            Venue Profile
                                        </button>
                                        {!isLgUp && (
                                            <button
